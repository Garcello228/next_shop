...
import { setFinalOrder } from '@/state/Slice/orderToSend';
import { IProductInCart } from '../../../../next-auth';
import PostOrder from '../api/PostOrder';
import CartAllRemove from '../api/CartAllRemove';
import RemoveWis from '../api/RemoveWis';
...
function ContentData({ Payment, isOpen } : IContentData)
{
  ...
  async function onSubmit(data :  ICheckoutFormInputs)
  {
    ...
    if(isOpen === 1)
    {
      ...
      const { Discount, Subtotal, ...cleanOrderToSend } = orderToSend;
      dispatch(setFinalOrder(cleanOrderToSend));
      ...
      try {
        await PostOrder({
          orderToSend: {
            ...cleanOrderToSend,
            CartItems: cleanOrderToSend.CartItems ?? [],
            Total: cleanOrderToSend.Total ?? 0
          }
        });
        ...
        const RemoveWisl = Payment?.CartItems ?? [];
        ...
        <form className='content__data' id="my-form" onSubmit={handleSubmit(onSubmit)} noValidate>
              ...
              <div className='data__checkbox'>
                <label className="customCheckbox">
                 <input type="checkbox"  onChange={CheckboxClik} checked={checkbox} />
