...
import "./CartItemExplore.scss"
import PostCart from "@/zshared/api/PostCart";


function CartItemExplore ({ item }: { item : IProduct}) {
   ...
   async function AddBaskett()
   {
    ...
    function InfoID()
    {
        router.push(`/product/${item.id}?details=true&color=${isOpen}`);
        ///details?color=${isOpen}
    }
...
export default CartItemExplore